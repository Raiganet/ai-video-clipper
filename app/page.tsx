  ...
  const handleFindPreviews = async () => {
    ...
    try {
      ...
      if (settings.aiMode === "transcript") {
        setStatus("Mentranskripsi audio dengan AI...");
        try {
          ...
          console.warn("AI gagal, fallback ke pembagian otomatis:", aiError);
    ...
    <div className="min-h-screen bg-black text-white">
      <header className="border-b border-zinc-800">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center gap-2">
          ...
          <span className="bg-yellow-500/20 text-yellow-500 text-xs px-2 py-1 rounded-full">BETA</span>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 py-12">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 rounded-full px-4 py-2 mb-4">
            ...
            <span className="text-emerald-400 text-sm">Proses 100% di browser • Gratis & Aman</span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            Ubah video panjang jadi <span className="text-emerald-500">klip viral</span>
          </h1>
          <p className="text-zinc-400 text-lg">Upload video, biarkan AI mencari momen terbaik, dan potong langsung di perangkatmu.</p>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 mb-8">
          <div className="flex gap-2 mb-6">
            <button
              ...
              className={`flex-1 py-3 px-4 rounded-lg flex items-center justify-center gap-2 transition-all ${
                activeTab === "youtube" ? "bg-red-600 text-white" : "bg-zinc-800 text-zinc-400 hover:bg-zinc-700"
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-3 px-4 rounded-lg flex items-center justify-center gap-2 transition-all ${
                activeTab === "upload" ? "bg-emerald-600 text-white" : "bg-zinc-800 text-zinc-400 hover:bg-zinc-700"
              }`}
            >
          ...
          <div className="mt-8">
            <label className="text-sm text-zinc-400 mb-3 block">GAYA CAPTION <span className="text-zinc-600">(Clean disarankan)</span></label>
        ...
        {clips.length > 0 && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 mb-8">
            <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
              <Scissors className="w-5 h-5 text-emerald-500" /> Preview Klip ({clips.length})
            </h3>
              ...
              {clips.map((c) => (
                <button
                  ...
                  className={`text-left p-4 rounded-lg border transition-all ${
                    selectedId === c.id ? "border-emerald-500 bg-emerald-500/10" : "border-zinc-700 bg-zinc-800 hover:border-zinc-600"
                  }`}
                >
                  <div className="font-semibold text-sm mb-1">{c.title}</div>
                  <div className="text-xs text-zinc-400">{fmt(c.start)} – {fmt(c.start + c.duration)}</div>
                  <div className="text-xs mt-3 flex items-center gap-2">
                    {c.processing ? (
                      <span className="text-emerald-400 flex items-center gap-1"><Loader2 className="w-3 h-3 animate-spin" /> Memproses...</span>
                    ) : c.blobUrl ? (
            ...
            {selectedClip?.blobUrl && (
              <div className="p-4 bg-zinc-800/50 rounded-lg border border-zinc-700">
                <video src={selectedClip.blobUrl} controls className="w-full rounded-lg bg-black max-h-[400px]" />
                <button
